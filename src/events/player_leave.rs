...
impl EventHandler<PlayerLeaveEvent> for PlayerLeaveEventHandler {
    fn handle(
        ...
        let leave_message = event.leave_message.get_text();

        match lua.try_enter(|ctx| {
            let listeners = listeners_table(ctx, "player_leave");
            ...
            let msg_str = piccolo::String::from_slice(&ctx, leave_message.as_bytes());
            event_table.set(ctx, "leave_message", msg_str).ok();
            ...
            Ok(executors) => {
                for executor in &executors {
                    if let Err(e) = lua.execute::<()>(executor) {
                        tracing::error!("PLua event: failed to execute leave listener: {}", e);
                    }
                }
            }
            Err(e) => {
