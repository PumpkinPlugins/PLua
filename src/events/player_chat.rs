...
impl EventHandler<PlayerChatEvent> for PlayerChatEventHandler {
    fn handle(
        &self,
        _server: Server,
        event: EventData<PlayerChatEvent>,
        ...
        match lua.try_enter(|ctx| {
            let listeners = listeners_table(ctx, "player_chat");
            ...
            let msg_str = piccolo::String::from_slice(&ctx, message.as_bytes());
            event_table.set(ctx, "message", msg_str).ok();
            ...
            Ok(executors) => {
                for executor in &executors {
                    if let Err(e) = lua.execute::<()>(executor) {
                        tracing::error!("PLua event: failed to execute chat listener: {}", e);
                    }
                }
            }
            Err(e) => {
